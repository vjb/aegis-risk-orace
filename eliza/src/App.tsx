...
export default function App() {
    ...
    const handleIntent = async (content: any) => {
        console.log("Intent triggered:", content);
        ...
        if (!requestId) {
            console.error("No Request ID provided for polling");
            return;
        }
        ...
        const pollInterval = setInterval(async () => {
            try {
                // Fetch the forensic report generated by cre-listener
                const response = await fetch(`/reports/${requestId}.json`);
                if (response.ok) {
                    const report = await response.json();

                    clearInterval(pollInterval);
                    console.log(" Forensic Report Received:", report);

                    setWorkflowStatus("ANALYZING");
                    setCurrentStep(2);

                    // Update UI with REAL data from report
                    setScanData(report);
                    setVerdict(report.status === "SAFE" ? "SAFE" : "UNSAFE");

                    // Short delay for visual effect of "ANALYZING" before "COMPLETE"
                    setTimeout(() => {
                    ...
                    setTimeout(() => {
                        setWorkflowStatus("COMPLETE"); // STAY in COMPLETE state
                        setCurrentStep(4);

                        // Inject Final Success Message with Link
                        setMessages(prev => [...prev, {
                            id: Date.now().toString(),
                            role: 'agent',
                            content: `✅ **SETTLEMENT CONFIRMED**\n\nThe Vault has successfully executed the transaction.\n\n[View on Tenderly Explorer](https://dashboard.tenderly.co/explorer/vnet/f6ccf890-f623-4060-940b-cf865824d26a/tx/${report.txHash || requestId})`
                        }]);
                    }, 4000);
                }
            } catch (e) {
                console.log("Waiting for audit report...");
            }
        }, 1000);

        // Safety timeout to stop polling after 60s
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.content && data.content.requestId) {
                handleIntent(data.content);
            }
