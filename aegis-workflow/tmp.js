...
var requestSchema = exports_external.object({
  tokenAddress: exports_external.string().min(1, "Token address is required"),
  chainId: exports_external.string().min(1, "Chain ID is required"),
  askingPrice: exports_external.string().optional(),
  amount: exports_external.string().optional(),
  userAddress: exports_external.string().optional()
});
var brainHandler = async (runtime2, payload) => {
  runtime2.log("\uD83E\uDDE0 [AEGIS] Starting Intelligent Analysis...");
  let requestData;
  if (!payload.input || payload.input.length === 0) {
    runtime2.log("\uD83D\uDCDD No payload provided, using defaults");
    requestData = {
  ...
  const openaiKey = runtime2.config.openaiApiKey || await runtime2.getSecret("OPENAI_API_KEY");
...
`
  ...
  const aiResponse = httpClient.sendRequest(runtime2, {
  ...
  if (ok(aiResponse)) {
    ...
    runtime2.log(`⚠️ AI API failed (Status: ${statusCode})`);
    runtime2.log(`❌ Error: ${errorBody.substring(0, 500)}`);
    aiResult = {
